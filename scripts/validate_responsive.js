...
const PUBLIC_HTML = [
  'index.html',
  CHAPTERS.hub.catalog,
  ...CHAPTERS.chapters.map((chapter) => chapter.file),
  ...CHAPTERS.hub.pages,
  '404.html',
];
const VIEWPORTS = [
...
const SCREENSHOT_DIR = path.join(ARTIFACTS_DIR, 'responsive');
const REPORT_PATH = path.join(ARTIFACTS_DIR, 'responsive-report.json');
...
async function validatePage(browser, baseUrl, file, viewport) {
  ...
  if (name) {
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, name), fullPage: file === 'index.html' });
  }
...
async function validateNotFound(browser, baseUrl) {
  ...
  const page = await context.newPage();
  const response = await page.goto(`${baseUrl}/not-found-test`, { waitUntil: 'domcontentloaded', timeout: 30000 });
...
async function main() {
  ...
  try {
    ...
    notFound = await validateNotFound(browser, baseUrl);
    console.log(`${notFound.failures.length ? 'FAIL' : 'PASS'} HTTP 404 /not-found-test${notFound.failures.length ? ` — ${notFound.failures.join('; ')}` : ''}`);
  } finally {
    await browser.close();
  ...
  const report = {
    ...
    notFound,
    screenshots: [
      'artifacts/responsive/index-desktop.png',
      'artifacts/responsive/index-320.png',
      'artifacts/responsive/ch17-360.png',
      'artifacts/responsive/prompts-360.png',
    ],
    records,
  };
  fs.writeFileSync(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`);
  console.log(`Report: ${path.relative(ROOT, REPORT_PATH)}`);
  console.log(`Matrix: ${report.summary.passedCases}/${report.summary.matrixCases} passed; 404 ${notFound.failures.length ? 'failed' : 'passed'}`);
